...
source(paste(pp,"/script/megafauna vetting.R", sep=""))
...
make.edge<-function(keep){
  ...
  perms<-merge(perms,keep[,c("SciName","mean.mass.kg","eat.plant","eat.invert","eat.vert","eat.FishInv")],by.x="from",by.y="SciName",all.x=T)
  colnames(perms)<-c("from","to","fmasskg","fplant","finvert","fvert","fFishInv")
  perms<-merge(perms,keep[,c("SciName","mean.mass.kg","eat.plant","eat.invert","eat.vert","eat.FishInv","Class")],by.x="to",by.y="SciName",all.x=T)
  colnames(perms)<-c("to","from","fmasskg","fplant","finvert","fvert","fFishInv","tmasskg","tplant","tinvert","tvert","tFishInv","Cpred")
...
#Build networks##########################################
#For each predator
#1) randomly select the proportion of links to keep/remove
#2) round off number of links
...
diet_oth <- read.csv(paste(pp,"/data/Table S5 diet breadth.csv", sep=""))
herb_b <- diet_oth[diet_oth$diet.group=="herb",]
hd <- density(herb_b$breadth)
inv_b <- diet_oth[diet_oth$diet.group=="inv",]
id <- density(inv_b$breadth)
FishInv_b <- diet_oth[diet_oth$diet.group=="pisc",]
fd <- density(FishInv_b$breadth)

#Old versions for herbivores, invertivores, and aquatic feeders:
...
lkp.ap<-function(y, net) #y = number prey predicted; net = edge list  
{     
  ...
  flinks<-round(sample(FishInv_b$breadth, 2*dim(fishi)[1], replace=TRUE) + rnorm(2*dim(fishi)[1], 0, fd$bw))
  flinks<-flinks[!(flinks<1)&!(flinks>2*max(FishInv_b$breadth))] 
...
saveRDS(x80,paste("/Users/llew0024/Dropbox/trophic links/Docs/supp material/keepers/networks/x80nets.rds",sep=""))
